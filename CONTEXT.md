...
本地 Skill 内任意层级下，名称严格为 `.venv`、`venv`、`node_modules` 的目录或符号链接属于依赖子树，不参与扫描计数、来源识别和本地纳管的内容变更判断。同名普通文件和相似名称不排除；不会跟随依赖符号链接，也不会以依赖中的 SKILL.md 或 Git 元数据推断来源。扫描与计划统一使用按相对路径字节序排列的观察哈希。此规则不是文件删除或复制排除规则：外部 Local Link 迁移仍完整保留依赖文件和原始符号链接，执行时冻结完整物理快照用于复制一致性及回滚/恢复校验；Git Source Release 与已安装内容的完整性校验不变。旧观察规则生成的扫描缓存需重新扫描一次。
