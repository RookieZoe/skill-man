...
控制目录中的 Local Source 使用显式外部迁移：选择稳定外部父目录，预览原路径、目标路径及受影响入口，确认后迁移并登记为 Link。预览和取消不写入；不覆盖已有目标，失败回滚，撤销验证内容及身份。它不是 Home 内的 File Install；结果局部更新，不自动重新扫描。
迁移的目录选择、预览、确认、错误及结果集中在一个弹窗。忽略只更新持久化忽略名单，不使其它候选的扫描证据失效；计划和执行均复查忽略名单，已忽略技能不可继续纳管。真实文件系统、配置或 Home 变化仍会使报告失效，不以局部 UI 更新绕过校验。

本地 Skill 内任意层级下，名称严格为 `.venv`、`venv`、`node_modules` 的目录或符号链接属于依赖子树，不参与扫描计数、来源识别和本地纳管的内容变更判断。同名普通文件和相似名称不排除；不会跟随依赖符号链接，也不会以依赖中的 SKILL.md 或 Git 元数据推断来源。扫描与计划统一使用按相对路径字节序排列的观察哈希。此规则不是文件删除或复制排除规则：外部 Local Link 迁移仍完整保留依赖文件和原始符号链接，执行时冻结完整物理快照用于复制一致性及回滚/恢复校验；Git Source Release 在物化时仅忽略 Skill 内名称严格为 `.venv`、`venv`、`node_modules` 且目标为绝对路径或按路径计算越出该 Skill 根目录的软链；预览列出忽略路径并提示可能需要在本地副本或项目目录中重建依赖。普通目录、文件、内部链接与其它不安全链接保留原校验。Git tree summary 保留远端树事实，安装快照哈希覆盖实际物化内容；Update/Restore 使用相同规则，Undo/recovery 使用冻结快照，已安装内容仍完整校验。旧观察规则生成的扫描缓存需重新扫描一次。
