...
#112 继续承担完整组合验收，包括 多屏/缩放、屏幕边缘、托盘实际菜单宽度、Dock 两种偏好、故障注入与正式更新通道证据。现有 CUA 原生 app surface 未暴露状态栏图标，不能将实际托盘左/右键、子菜单和屏幕边缘测试记为通过。真实下载/安装不通过伪造正式发布来验证。
...
## 规格同步与原生冒烟补测（2026-09-14，自动化阶段历史记录）
...
## 用户手工验收确认与 #113 结项（2026-09-14）

用户在上述自动化补测后明确回复：“我验证通过，你可以回填证据，然后关闭 #113”。该确认对应上一轮列出的待手工项目：原生托盘左/右键、子菜单、Esc 与外部点击收起、Dock 两种偏好（包括图标显隐）、双屏菜单交互。以上项目记为用户手工验收通过，补齐 #113 的原生冒烟证据；前节未验证结果保留为自动化阶段历史，不再表示这些项目仍待用户验证。

证据来源是用户本次确认，不是代理自动化截图。用户没有另报构建号、逐项操作日志或截图，因此沿用本轮测试上下文（实现 `4b0bcb2` / Skill Man 0.1.3），不声称独立核验了用户实际运行二进制。此确认不扩展为 所有键盘组合、不同缩放组合、故障注入或真实下载/安装均通过；这些更细的组合验收仍由 #112 记录。

#113 按用户明确授权结项；#112 和父规格 #108 不在本次关闭范围。代码已在 main、未发版。已有完整本地 CI 与原生构建证据保持有效；本次仅更新验收记录，不改变产品代码。
