...
| [实施：Source Tracking Policy 与 immutable Source Transition](https://github.com/RookieZoe/skill-man/issues/92) | 已交付（issue-92-source-tracking-transition）：确定性 policy 求值（provider Release→stable SemVer→reachable tag→HEAD；显式 channel/fixed tag/commit/branch/HEAD）、v9 完整 added/current/removed manifest 预览、Repository Ref Conflict/Ownership Split、source.json+Catalog 的 immutable release、`skills/git/<remote_id>/<skill_id>` namespace、单一 lock full-file CAS 提交点、journal v2 冻结 policy/claims/manifest、pre/post-CAS 回滚与 roll-forward、SQLite v9 transition/promotion store、Legacy Promotion 复用同一 transition（bindings audit-only，Undo 还原）、DTO/commands/React 竖切；Source Update 写路径封闭（#93 恢复）；fault 测试覆盖 policy fallback/namespace/CAS race-crash/undo guard | Source Ownership Commit Point 前整体 rollback；之后完整 release roll-forward |
