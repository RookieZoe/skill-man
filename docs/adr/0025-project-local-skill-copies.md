...
状态：Accepted（设计已确认；#101–#104 已实现；原生验收状态见验证记录）
...
- [#104](https://github.com/RookieZoe/skill-man/issues/104) 已贯通 Library 多选、每个 Directory Identity 的副本来源选择和同一 operation 内的多副本依赖 journal。批量 Undo / 启动恢复按各组安全性独立处理，覆盖确认沿用 #103。验证及原生验收状态见 [#104 验证记录](../project-batch-verification.md)。
