...
#[test]
fn real_document_reads_distinguish_unreadable_broken_skills_from_empty_markdown() {
    let home = common::BoundTestHome::new();
    home.seed_standard_library();
    let api = CatalogApi::new(CatalogService::new(home.runtime.clone()));
    let detail = api.inspect_skill("skill-authoring".into()).unwrap();
    assert!(detail.document_available);
    assert_eq!(
        detail.skill_markdown,
        "# Skill authoring\n\nA precise workflow for building maintainable Agent Skills.\n"
    );
    let document = std::path::Path::new(&detail.final_entity_path).join("SKILL.md");
    std::fs::write(&document, "").unwrap();
    assert!(
        api.inspect_skill("skill-authoring".into())
            .unwrap()
            .document_available
    );
    std::fs::remove_file(&document).unwrap();
    assert!(api.inspect_skill("skill-authoring".into()).is_err());
    ...
    let path = api
        .inspect_skill("broken-link".into())
        .unwrap()
        .final_entity_path;
    std::fs::remove_file(std::path::Path::new(&path).join("SKILL.md")).unwrap();
    let broken = api.inspect_skill("broken-link".into()).unwrap();
    assert!(!broken.document_available);
}
