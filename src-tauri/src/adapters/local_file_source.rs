...
// These are transport guards, not Skill validation limits. Import Core independently
// validates every discovered candidate against the stricter per-Skill policy.
...
fn stage_zip(original_path: &Path, extraction_root: &Path) -> Result<(), SourceError> {
    ...
    let archive_file = fs::File::open(original_path).map_err(|source| SourceError::Io {
        operation: "open ZIP file Import source",
    ...
    for index in 0..archive.len() {
        ...
        {
            return Err(SourceError::Validation(format!(
                "file Import ZIP contains an unsafe path: {}",
                entry.name()
            )));
        }
        ...
        if entry.size() > MAX_STAGED_ENTRY_BYTES {
            return Err(SourceError::Validation(format!(
                "file Import ZIP entry exceeds the transport limit: {}",
                entry.name()
            )));
        }
        ...
        loop {
            ...
            if entry_bytes > MAX_STAGED_ENTRY_BYTES || total_bytes > MAX_STAGED_SOURCE_BYTES {
                return Err(SourceError::Validation(
                    "file Import ZIP exceeds the transport limits".into(),
                ));
            }
    ...
    // Links are materialized but never followed here. Import Core validates their
    // relative targets and containment before the staged tree is eligible to apply.
